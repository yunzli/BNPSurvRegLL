...
Distributions.median(d::LogLogistic) = d.θ
function Distributions.mean(d::LogLogistic)
	if d.ϕ ≤ 1
...
function Distributions.mode(d::LogLogistic)
	if d.ϕ ≤ 1
...
function Distributions.var(d::LogLogistic)
	if d.ϕ ≤ 2
...
#### Evaluation
function Distributions.pdf(d::LogLogistic, x::Real)
    if x ≤ zero(0)
        z = zero(x)
    else
...
function Distributions.logpdf(d::LogLogistic, x::Real)
    if x ≤ zero(0)
        z = log(zero(x))
    else
...
function Distributions.cdf(d::LogLogistic, x::Real)
    y = (x/d.θ)^d.ϕ
    z = y / (y + 1)
    return x ≤ 0 ? 0 : z
end

function Distributions.logcdf(d::LogLogistic, x::Real)
    y = (x/d.θ)^d.ϕ
	z = log(y) - log(y+1) 
	return x ≤ 0 ? -Inf : z
end

function Distributions.ccdf(d::LogLogistic, x::Real)
    y = (x/d.θ)^d.ϕ
	z = 1 / (y + 1) 
	return x ≤ 0 ? 1 : z 
end

function Distributions.logccdf(d::LogLogistic, x::Real)
    y = (x/d.θ)^d.ϕ
	z = - log(y+1)
	return x ≤ 0 ? 0 : z 
end
...
#### Sampling
function Base.rand(rng::AbstractRNG, d::LogLogistic)
    u = rand(rng)
    r = u / (1 - u)
    return r^(1/d.ϕ)*d.θ
end
